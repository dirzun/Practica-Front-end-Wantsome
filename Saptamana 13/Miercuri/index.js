//exercitiul 1
  ...
  */
  ...
  const dateUsers = fetch(url).then(response => response.json())
  .then(x=> console.log(x.filter(id => id.id > 5).map((y) =>{return { "username" : y.username, "city" : y.address.city} })));
...
fetch(url3)
  .then(resp => resp.json())
  .then(obj => console.log(obj.items.filter(x=>x.volumeInfo.pageCount>230).filter(y => y.volumeInfo.authors.length >= 3)));
